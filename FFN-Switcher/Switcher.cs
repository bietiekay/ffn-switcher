...
namespace FFN_Switcher
{
    public class Switcher
    {
        ...
        public void Run()
        {
            ...
            httpServer = new FFN_Switcher.HTTP.HttpServer(AppSettings);
            ...
            TeamspeakProcessor tsProcessor = new TeamspeakProcessor(AppSettings);
            Thread teamspeakThread = new Thread(new ThreadStart(tsProcessor.TeamSpeakProcessorThread));
            ...
            {
                FFN_Switcher.FFN_Switcher_Console.Shutdown();
                done = true;
            };
            ...
            while (!done)
            {
                try
                {
                    if (tsProcessor.done)
                    {
                        ...
                        tsProcessor = new TeamspeakProcessor(AppSettings);
