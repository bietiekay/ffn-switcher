...
namespace FFN_Switcher.HTTP
{
	...
	public class HttpProcessor
	{
		...
		public HttpProcessor(Socket s)
		{
			...
			this.s = s;
			docRootFile = new FileInfo(FFN_Switcher.Properties.Settings.Default.WebserverDocumentRoot);
		...
		public bool parseRequest()
		{
			...
			if (request == null)
			{
				if (FFN_Switcher.Properties.Settings.Default.WebserverVerboseLogging)
				{
					ConsoleOutputLogger.WriteLine("[HTTP] Keep-alive broken after " + numRequests + " requests");
				}
				return false;
			}
		...
		public void writeURL()
		{
			try
			{
				...
				querystring = "";
				url = original_url;
				if (FFN_Switcher.Properties.Settings.Default.WebserverVerboseLogging) ConsoleOutputLogger.WriteLine("[HTTP]: " + url);
						...
						else
						{
							...
							try
							{
								if (filename.EndsWith(".log"))
								{
									...
									{
										// 
										String Output = Template_Processor.ProcessHTMLTemplate(filename, querystring);
